...
class ClaudeProvider(
    ...
    override suspend fun generateResponse(prompt: String, systemPrompt: String, apiKey: String): String? = withContext(Dispatchers.IO) {
        ...
        try {
            // BUGFIX: "claude-3-5-sonnet-20241022" is a 2024-era snapshot; current
            // default is claude-sonnet-5. Centralized in ProviderDiagnosticService.
            val json = JSONObject().apply {
                put("model", com.example.data.api.diagnostics.ProviderDiagnosticService.CLAUDE_MODEL)
                put("max_tokens", 300)
            ...
            android.util.Log.w("ClaudeProvider", "generateResponse failed: ${e.message}")
        }
        return@withContext null
    }
}
